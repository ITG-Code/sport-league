...
class Select extends dbSetup{
  ...
  public function getFullTeamName($tid){
    ...
    *
    **/
    $stmt = $this->db->prepare("SELECT org.name AS org_name, team.name AS team_name FROM team JOIN org ON team.org_id = org.id WHERE org.id = ?");
  ...
  public function getMVP($team){
    //FIX
    $stmt = $this->db->prepare("SELECT person_id, person.fname AS first_name, person.sname AS sur_name, person.social_sec AS birthDate, height AS player_heigth, weight AS player_weight, avatar AS player_avatar, SUM( totalGoals ) AS total FROM ( SELECT game_person_link.id AS game_person_id, team_person_link.person_id AS person_id, team_person_link.avatar, team_person_link.height, team_person_link.weight, game_person_link.game_id AS game_id, ( SELECT COUNT( * ) FROM goals WHERE goals.game_person_id = game_person_link.id ) AS totalGoals FROM game_person_link LEFT JOIN team_person_link ON game_person_link.team_person_id = team_person_link.id LEFT JOIN game ON game_person_link.game_id = game.id WHERE team_person_link.team_id =? ) AS allGoals LEFT JOIN person ON person.id = person_id GROUP BY person_id ORDER BY total DESC ");
    $stmt->bind_param('i', $team);
    $stmt->execute();
    $result = $stmt->get_result();
    $player = $result->fetch_object();
    return new Player($player->person_id, $player->first_name, $player->sur_name, $player->birthDate, $player->player_heigth, $player->player_weight, $player->player_avatar);
  }
  ...
  public function getTeamPlayers($team){
    ...
    while ($row = $result->fetch_object()) {
      array_push($teamPlayers, new Player($row->person_id, $row->first_name, $row->sur_name, $row->brithdate, $row->player_heigth, $row->player_weight, $row->player_avatar));
    }
    return $teamPlayers;
  }
